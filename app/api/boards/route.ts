...
export async function POST(request:Request) {
  ...
  if (!subcategoryId || !title || !content) return NextResponse.redirect(new URL(`/boards/write?board=${subcategoryId}&error=1`, request.url), 303);
  ...
  if (error || !post) return NextResponse.redirect(new URL(`/boards/write?board=${subcategoryId}&error=1`, request.url), 303);
  return NextResponse.redirect(new URL(`/boards/${post.id}?board=${subcategoryId}`, request.url), 303);
}
