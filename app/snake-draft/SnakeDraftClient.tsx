...
type Member={id:string;nickname:string;main_line:string|null;sub_line:string|null;match_tier:number|null};
...
export default function SnakeDraftClient({isStaff}:{isStaff:boolean}){
 const [state,setState]=useState<State|null>(null); const [selected,setSelected]=useState<string[]>([]); const [message,setMessage]=useState(""); const [busy,setBusy]=useState(false);
 ...
 return <div className="snake-page">
  <section className="card snake-head"><div><span>INHOUSE DRAFT</span><h1>🐍 스네이크 픽</h1><p className="muted">운영진이 참가자를 추가하고 현재 픽 차례 팀으로 선수를 드래그합니다.</p></div><div className="snake-turn"><small>현재 픽</small><b>{state.teams.find(t=>t.team_no===state.settings.expected_team)?.name||`${state.settings.expected_team}팀`}</b><em>#{state.settings.current_pick+1}</em></div></section>
  {message&&<div className="error">{message}</div>}
  {isStaff&&<section className="card snake-controls"><div className="snake-control-row"><label>팀 수<select value={state.settings.team_count} onChange={e=>act({action:"configure",teamCount:Number(e.target.value)})} disabled={busy}>{[2,3,4].map(n=><option key={n} value={n}>{n}팀</option>)}</select></label><button className="button" onClick={()=>confirm("픽 배정을 전부 초기화할까요?")&&act({action:"reset"})}>픽 초기화</button></div><div className="snake-member-picker"><select multiple value={selected} onChange={e=>setSelected(Array.from(e.target.selectedOptions).map(o=>o.value))}>{available.map(m=><option key={m.id} value={m.id}>{m.nickname} · {m.main_line||"미정"}/{m.sub_line||"미정"} · {m.match_tier?`${roman[m.match_tier]}티어`:"미정"}</option>)}</select><button className="button primary" disabled={!selected.length||busy} onClick={async()=>{if(await act({action:"addMembers",memberIds:selected}))setSelected([])}}>선택 명단 추가</button></div></section>}
