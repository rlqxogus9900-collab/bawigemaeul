...
export default async function P() {
  ...
  return <>
    <section className="card">
      <h1>공지 관리</h1>
      <form className="form" action="/api/admin/notices" method="post">
        <input name="title" placeholder="공지 제목" required />
        <textarea name="content" placeholder="공지 내용" rows={6} required />
        <BoardImageUploader />
        <label><input name="is_pinned" type="checkbox" /> 상단 고정</label>
        <button className="button primary">공지 등록</button>
      </form>
    </section>

    <section className="card" style={{ marginTop: 14 }}>
      <h2>등록된 공지</h2>
      {data?.map(notice => <article className="notice-item" key={notice.id}>
        <form className="form" action={`/api/admin/notices/${notice.id}`} method="post">
          <input name="title" defaultValue={notice.title} required />
          <textarea name="content" defaultValue={notice.content} rows={4} required />
          <BoardImageUploader initialUrls={Array.isArray(notice.image_urls) ? notice.image_urls : []} />
